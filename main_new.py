...
class Qnetwork():
    def __init__(self, h_size, action_num):
        ...
        self.scalarInput = tf.compat.v1.placeholder(shape=[None, 3600, 3], dtype=tf.float32)
        self.domainActionInput = tf.compat.v1.placeholder(shape=[None, 1], dtype=tf.float32)
        self.legal_actions = tf.compat.v1.placeholder(shape=[None, action_num], dtype=tf.float32)
        ...
        self.conv1 = tf.keras.layers.Conv2D(inputs=self.imageIn, num_outputs=32, kernel_size=[4, 4], stride=[2, 2],
                                 padding='VALID', activation_fn=self.relu, biases_initializer=None)
        self.conv2 = tf.keras.layers.Conv2D(inputs=self.conv1, num_outputs=64, kernel_size=[2, 2], stride=[1, 1],
                                 padding='VALID',
                                 activation_fn=self.relu, biases_initializer=None)
        self.conv3 = tf.keras.layers.Conv2D(inputs=self.conv2, num_outputs=128, kernel_size=[2, 2], stride=[1, 1],
                                 padding='VALID',
                                 activation_fn=self.relu, biases_initializer=None)
        ...
        self.Advantage = tf.subtract(self.Advantage, tf.reduce_mean(input_tensor=self.Advantage, axis=1, keepdims=True))\
                         /(tf.math.reduce_std(self.Advantage, axis=1, keepdims=True) + 1e-5)
        self.Value = tf.matmul(self.streamV0, self.VW)

        # Then combine them together to get our final Q-values.
        self.Qout0 = self.Value + tf.subtract(self.Advantage, tf.reduce_mean(input_tensor=self.Advantage, axis=1, keepdims=True))
        ...
        self.predict = tf.argmax(input=self.Qout, axis=1)
        ...
        self.Q = tf.reduce_sum(input_tensor=tf.multiply(self.Qout, self.actions_onehot), axis=1)
        #         self.Q = tf.reduce_sum(self.Qout, axis=1)

        self.safe_q = tf.compat.v1.placeholder(shape=[None, action_num], dtype=tf.float32)
        self.action_q = tf.compat.v1.placeholder(shape=[None, action_num], dtype=tf.float32)
        ...
        self.safe_penalty_scaler = tf.compat.v1.placeholder(shape=[None, 1], dtype=tf.float32)
        self.safe_penalty = tf.multiply(self.safe_penalty, tf.transpose(a=self.domainActionInput))
        ...
        self.loss = tf.reduce_mean(input_tensor=self.td_error) + tf.reduce_sum(input_tensor=self.safe_penalty) * 1e-4 * tf.reduce_sum(input_tensor=self.safe_penalty_scaler)
...
# record the loss
tf.compat.v1.summary.scalar('Loss', mainQN.loss)
...
s_writer = tf.compat.v1.summary.FileWriter(path + '/train', sess.graph)
